package com.ms365.middleware.proyectosarchivos.utilities;
...
public class DateUtil {
  ...
  public static Date getTimeStamp(Date date){
	  return getTimeStamp(format(Constants.Format.Date.DAY_MONTH_YEAR, date),
			                  format(Constants.Format.Time.HOURS_MINUTES_SECONDS, date));
  }
  ...
  public static boolean validateDateValue(String value){
  	boolean retValue = false;
		...
		try {
			DateFormat dateFormat = new SimpleDateFormat(Constants.Format.Date.DAY_MONTH_YEAR);
  ...
  public static boolean validateTimeValue(String value){
  	...
  	if (value.length() == 5) value += ":00";
		...
		try {
			DateFormat dateFormat = new SimpleDateFormat(Constants.Format.Time.HOURS_MINUTES_SECONDS);
			dateFormat.setLenient(false);
			String out = "" + dateFormat.parse(value);
			retValue = true;
		}
